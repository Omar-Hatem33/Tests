 ...
 *
 * <p>The "package com.testgen.uber" header is rewritten by the grader's
...
public abstract class TestBase {
    ...
    protected String driverServiceUrl;
    protected String rideServiceUrl;
    ...
    protected String s3RideEntity() {
        String v = (String) theme().get("s3RideEntity");
        if (v == null) throw new IllegalStateException("theme.json missing 's3RideEntity'");
        return v;
    }
     ...
     *  manifest.mongoCollections. For Uber this is "ride_events". */
    ...
    /** S3 recommendation graph: catalog-side node label (e.g., "Driver"
     *  for Uber). Strict spec-name lookup against manifest.neo4jNodes. */
     ...
     *  (e.g., "ORDERED_FROM" for Uber). Read straight from theme.json —
    ...
    /** S4 entity managed by the tracking/tracking service (e.g., "Tracking"
     *  for Uber, "Ride" for Uber). Read from theme.json. */
     ...
     *  manifest.mongoCollections. For Uber this is "tracking_events". */
     ...
     *  manifest.cassandraTables. For Uber this is "tracking_tracking_events". */
     ...
     *  (e.g., "trackingId"). Read from theme.json. */
     ...
     *  Uber, sourced from PG via cross-service SQL). Read from theme.json. */
     ...
     *  Uber, "Transaction" for Amazon). Read from theme.json. */
     ...
     *  manifest.mongoCollections. For Uber this is "payment_audit_trail". */
    ...
    /** S5 service cache-key prefix (e.g., "checkout-service" for Uber,
    ...
    private Object _defaultValueFor(java.util.Map<String, Object> col) {
        ...
        switch (javaType) {
            case "String": {
                // For unique-constrained fields, append nonce to avoid 409s
                // across test classes that share the database. Other fields
                // keep deterministic defaults so verification queries work.
                String base = "Default " + fieldName;
                if (Boolean.TRUE.equals(col.get("unique"))
                        || "email".equals(fieldName) || "phone".equals(fieldName)
                        || "licenseNumber".equals(fieldName) || "license_number".equals(fieldName)) {
                    return base + "_" + nonce();
                }
                return base;
            }
     ...
     *   crudReadPath("Driver") → "/api/drivers/{driverId}/menu-items/{id}"
     *
     ...
     *   crudCollectionPath("Driver") → "/api/drivers/{driverId}/menu-items"
     */
    ...
    // S3 ride table that points to the S2 catalog table — robust to whatever
    // the student named that column. _seedTableRow / _seedS2Catalog /
    // _seedS3Rides / _seedBaselineUser are generic INSERT primitives that work
    // off information_schema metadata + per-type defaults; the 7 non-Uber
    // themes orchestrate them inside autoSeedBaselineData(). Uber's own
    ...
    // with whatever Uber's ride-→-driver FK is named.

    protected String s2CatalogFkColumn() {
        String rideTable   = tableName(s3RideEntity());
        ...
        try {
            return jdbc.queryForObject(
              ...
              + "ORDER BY kcu.ordinal_position LIMIT 1",
                String.class, rideTable, catalogTable);
        } catch (DataAccessException e) {
            throw new AssertionError(
                "No FK from ride table '" + rideTable
              + "' to catalog table '" + catalogTable
              + "'. The S3 ride entity must have a relation to the S2 catalog entity. "
              + e.getMessage());
        }
    }
    ...
    /** Cache: PG enum udt → labels (case-sensitive, in declaration ride). */
    ...
    private Object _defaultForPgColumn(String dataType, String udt, String column) {
        ...
        switch (dataType) {
            ...
            case "smallint": case "integer": case "bigint":
                ...
                // Defaulting "totalRatings"/"rideCount" to 1 corrupts running-
     ...
     *    3.  `(<col> IN ('A', 'B'))`
     *  Returns a {column → [valid labels in declaration ride]} map.
    ...
    protected void _seedS3Rides(long userId) {
        String table = tableName(s3RideEntity());
        String fk;
    ...
    /**
     * Pick the first manifest entity (in declared ride) that is NOT
     ...
     */
    /**
     * Returns the correct service base URL for a given entity class, based on
     * the entity's package name in the manifest.
     */
    @SuppressWarnings("unchecked")
    protected String serviceUrlForEntity(String entityClass) {
        ...
        if (entities != null) {
            for (java.util.Map<String, Object> e : entities) {
                if (!entityClass.equals(e.get("className"))) continue;
                String pkg = (String) e.get("packageName");
                if (pkg == null) break;
                if (pkg.contains("driver") || pkg.contains("catalog") || pkg.contains("restaurant"))
                    return driverServiceUrl;
                if (pkg.contains("ride") || pkg.contains("order"))
                    return rideServiceUrl;
                if (pkg.contains("location") || pkg.contains("delivery"))
                    return locationServiceUrl;
                if (pkg.contains("payment") || pkg.contains("checkout"))
    ...
    protected void initBase() {
        ...
        // Defaults match the canonical Uber docker-compose port allocation.
        userServiceUrl     = envOr("USER_SERVICE_URL",     "http://localhost:8081");
        driverServiceUrl  = envOr("CATALOG_SERVICE_URL",  envOr("RESTAURANT_SERVICE_URL", "http://localhost:8082"));
        rideServiceUrl    = envOr("ORDER_SERVICE_URL",    "http://localhost:8083");
        locationServiceUrl = envOr("DELIVERY_SERVICE_URL", "http://localhost:8084");
        paymentServiceUrl = envOr("CHECKOUT_SERVICE_URL", "http://localhost:8085");
        String dbUrl = envOr("SPRING_DATASOURCE_URL", "jdbc:postgresql://localhost:5432/foodtrackingdb");
                ...
                if (mongoClient == null) {
                    ...
                    try {
                        ...
                        if (dbName == null || dbName.isBlank()) {
                            dbName = envOr("SPRING_DATA_MONGODB_DATABASE", "uber");
                        }
        ...
        if (cassandra == null) {
            synchronized (_CASSANDRA_INIT_LOCK) {
                if (cassandra == null) {
                    ...
                    String casKs   = envOr("SPRING_CASSANDRA_KEYSPACE_NAME", "foodtrackingks");
                    try {
    ...
    protected String columnByField(String entityClass, String... fieldNames) {
        ...
        // when the manifest scanner emits {fieldName="driver",
        // columnName="driver"} for a @ManyToOne field that Hibernate
        // actually maps to "driver_id" (because @JoinColumn(name=...)
        // wasn't read), the candidate "driver" doesn't exist in PG. We
        // then try "driver_id" (Hibernate's default FK column suffix)
     ...
     *  `@ManyToOne @JoinColumn(name="driver_id") Driver driver`
     *  ends up in the manifest as {fieldName="driver", columnName="driver"}
     *  but PG actually has the column as "driver_id". This validator
     ...
     *  ride_date, created_at, transaction_date — but NOT updated_at, which is
    ...
    protected void setAllDateColumns(String table, long rowId, java.sql.Timestamp ts) {
        ...
        if (dateCols.isEmpty()) {
            throw new AssertionError(
              ...
              + "S3-F10 dashboard date-range filtering needs an rideDate or createdAt column. "
              + "Add @Column private LocalDateTime rideDate; or rely on the baseline createdAt.");
        }
    ...
    /** Read the rideCount property on the first matching ORDERED_FROM-style
     *  relationship between (fromLabel:fromId) and (toLabel:toId). Returns
     *  -1 if no edge or driver is null. */
    protected long neo4jRelRideCount(String fromLabel, long fromId,
                                      String relType,
                                      String toLabel,   long toId) {
        if (neo4j == null) return -1L;
        java.util.List<java.util.Map<String, Object>> rows = neo4jExec(
            "MATCH (a:`" + fromLabel + "` {id: $fromId})-[r:`" + relType
          + "`]->(b:`" + toLabel + "` {id: $toId}) RETURN r.rideCount AS oc LIMIT 1",
            java.util.Map.of("fromId", fromId, "toId", toId));
    ...
    /** Read a string-typed relationship property (e.g. "lastRideDate" if
    ...
    // writes; S4-F12 verifies clustering-rideed reads. Tests must guard
    ...
    /** Count rows in a partition (e.g., all tracking events for one tracking).
     ...
     *  table's clustering ride, which spec mandates as DESC by event-time).
    ...
    // URL fields below — userServiceUrl, driverServiceUrl,
    // rideServiceUrl, locationServiceUrl, paymentServiceUrl — picked
    // per the section comment above the test class's @Tag (e.g.
    // "TC54 — S3-F10 …" → rideServiceUrl).
    //
     ...
     *  return it as a JsonNode. Spec-compliant per Uber M2 §5.2 — the
    ...
    protected void truncateBeforeEach(org.junit.jupiter.api.TestInfo info) {
        ...
        // drivers + menu items + rides + deliveries + payments + coupons).
        ...
        // (and caused real false positives — e.g., baseline EGYPTIAN driver
     ...
     *    S2F* → catalog-service    (drivers / menu items)
     *    S3F* → ride-service
     *    S4F* → tracking-service
     *    S5F* → checkout-service   (payments, coupons, refunds)
    ...
    private void routeBaseUrl(org.junit.jupiter.api.TestInfo info) {
        ...
        if      (cls.startsWith("S2F")) { BASE_URL = driverServiceUrl;   return; }
        else if (cls.startsWith("S3F")) { BASE_URL = rideServiceUrl;     return; }
        else if (cls.startsWith("S4F")) { BASE_URL = locationServiceUrl; return; }
        else if (cls.startsWith("S5F")) { BASE_URL = paymentServiceUrl;  return; }
        else if (cls.startsWith("S1F") || cls.startsWith("CC")) {
            BASE_URL = userServiceUrl; return;
        }
        // TC{N} prefix fallback
        int tc = -1;
        ...
        else if (tc >= 35  && tc <= 53)  BASE_URL = driverServiceUrl;
        else if (tc >= 54  && tc <= 99)  BASE_URL = rideServiceUrl;
        else if (tc >= 100 && tc <= 135) BASE_URL = locationServiceUrl;
        else if (tc >= 136 && tc <= 190) BASE_URL = paymentServiceUrl;
    ...
    // ─── auto-seed hooks (per-test baseline data) — Uber ───
    //
    ...
    //     ride so /rides/recommendations?userId=1 finds data. User
    //     id=4 has 5 rides so /users/preferences/dietary?minRides=5
    //     matches. User id=3 has dietary=VEGETARIAN preferences.
    //   * Drivers — 4 rows spanning ITALIAN/EGYPTIAN × OPEN/CLOSED
    //     with ratings 3.5..4.8 so minRating/maxRating filters match.
    //   * Rides — 8 rows: dates spread Jan–Feb 2026, statuses spanning
    ...
    //   * Coupons — 3 rows: PERCENTAGE × 2 + FIXED × 1.
    //   * payment_coupons junction — usage spread 4/2/1 across the 3
    //     coupons so /payments/coupons/top-used?limit=N returns ranked
    //     non-empty data.
    ...
    //     ride_date, tracking_address, ...) live in separate UPDATE
    ...
    private void _seedDriver(String name, String cuisine, String status) {
        String t = _tn("Driver"); if (t == null) return;
        java.util.Map<String, Object> ov = new java.util.HashMap<>();
        _put(ov, _cf("Driver", "name"),                              name);
        _put(ov, _cf("Driver", "cuisineType", "cuisine", "type"),    cuisine);
        _put(ov, _cf("Driver", "status"),                            status);
        _seedTableRow(t, ov);
    }

    private void _seedRide(long userId, long driverId, String status, double amount) {
        String t = _tn("Ride"); if (t == null) return;
        java.util.Map<String, Object> ov = new java.util.HashMap<>();
        _put(ov, _cf("Ride", "user", "customer"),               userId);
        _put(ov, _cf("Ride", "driver", "eatery"),           driverId);
        _put(ov, _cf("Ride", "totalAmount", "amount", "total"), amount);
        _put(ov, _cf("Ride", "status"),                         status);
        _seedTableRow(t, ov);
    }

    private void _seedTracking(long rideId, String status) {
        String t = _tn("Tracking"); if (t == null) return;
        java.util.Map<String, Object> ov = new java.util.HashMap<>();
        _put(ov, _cf("Tracking", "ride"),  rideId);
        _put(ov, _cf("Tracking", "status"), status);
        _seedTableRow(t, ov);
    }

    private void _seedPayment(long rideId, long userId, double amount, String status) {
        String t = _tn("Payment"); if (t == null) return;
        java.util.Map<String, Object> ov = new java.util.HashMap<>();
        _put(ov, _cf("Payment", "ride"),                        rideId);
    ...
    private void _seedCoupon(String code, String type, double value, int maxUses) {
        String t = _tn("Coupon"); if (t == null) return;
        java.util.Map<String, Object> ov = new java.util.HashMap<>();
        _put(ov, _cf("Coupon", "code"),                               code);
        _put(ov, _cf("Coupon", "discountType", "type"),               type);
        _put(ov, _cf("Coupon", "discountValue", "value", "amount"),   value);
        _put(ov, _cf("Coupon", "maxUses", "maxUsage", "usageLimit"),  maxUses);
        _put(ov, _cf("Coupon", "expiryDate", "validUntil"),           java.sql.Date.valueOf("2030-12-31"));
        _seedTableRow(t, ov);
    }
    ...
    private void _seedDriver(long driverId, String name, double price, String status) {
        String t = _tn("Driver"); if (t == null) return;
        java.util.Map<String, Object> ov = new java.util.HashMap<>();
        _put(ov, _cf("Driver", "driver", "eatery"), driverId);
        _put(ov, _cf("Driver", "name"),                 name);
        _put(ov, _cf("Driver", "price"),                price);
        _put(ov, _cf("Driver", "status"),               status);
        _seedTableRow(t, ov);
    }

    private void _seedRide(long rideId, long menuItemId, int quantity, double price, String status) {
        String t = _tn("Ride"); if (t == null) return;
        java.util.Map<String, Object> ov = new java.util.HashMap<>();
        _put(ov, _cf("Ride", "ride"),                     rideId);
        _put(ov, _cf("Ride", "menuItem", "item", "dish"),  menuItemId);
        _put(ov, _cf("Ride", "quantity"),                  quantity);
        _put(ov, _cf("Ride", "price"),                     price);
        _put(ov, _cf("Ride", "status"),                    status);
        _seedTableRow(t, ov);
    }

    /** payment_coupons junction. Discovered by walking information_schema for a
     *  table that has BOTH a payment FK column and an coupon FK column —
     *  resilient to students naming the join table payments_coupons,
     *  payment_coupon, etc. */
    private void _seedPaymentCoupon(long paymentId, long couponId) {
        String pTable = _tn("Payment");
        String oTable = _tn("Coupon");
    ...
    private void autoSeedBaselineData() {
        ...
        // finds the rich shape S1-F8 expects (user + preferences + 3 tracking
        ...
        // ── Drivers (4) ───────────────────────────────────────
        _seedDriver("Preseed Pizza Place", "ITALIAN",  "OPEN");
        _seedDriver("Preseed Koshari",     "EGYPTIAN", "OPEN");
        _seedDriver("Preseed Pasta House", "ITALIAN",  "CLOSED");
        _seedDriver("Preseed Falafel Hub", "EGYPTIAN", "OPEN");
        // Optional rating column (covers minRating / maxRating filters)
        String[] _ratingAliases = {"rating", "averageRating", "avgRating"};
        _tryUpdateCol("Driver", _ratingAliases, 4.5, 1L);
        _tryUpdateCol("Driver", _ratingAliases, 3.5, 2L);
        _tryUpdateCol("Driver", _ratingAliases, 4.0, 3L);
        _tryUpdateCol("Driver", _ratingAliases, 4.8, 4L);
        // Optional details jsonb (covers /drivers/details/search?key=note&value=true)
        String[] _detailsAliases = {"details", "metadata", "extra", "info"};
        _tryUpdateJsonb("Driver", _detailsAliases,
            "{\"note\":\"true\",\"spicy\":\"true\",\"vegetarian\":\"available\"}", 1L);
        _tryUpdateJsonb("Driver", _detailsAliases,
            "{\"note\":\"true\",\"halal\":\"true\"}", 2L);

        // ── Drivers (10) ────────────────────────────────────────
        // Distribution covers S2-F9 (unavailable list): driver 1 has
        // 2 AVAILABLE + 1 UNAVAILABLE; driver 2 has all AVAILABLE;
        // driver 3 has 3 UNAVAILABLE; driver 4 has 1 AVAILABLE.
        // Also referenced by S2-F8 (toggle), S3-F8 (add to ride), S3-F9
        // (ride details with item list).
        _seedDriver(1L, "Margherita Pizza",   80.0, "AVAILABLE");    //  1 — rest 1
        _seedDriver(1L, "Pepperoni Pizza",   100.0, "AVAILABLE");    //  2 — rest 1
        _seedDriver(1L, "Garlic Bread",       30.0, "UNAVAILABLE");  //  3 — rest 1
        _seedDriver(2L, "Koshari",            50.0, "AVAILABLE");    //  4 — rest 2
        _seedDriver(2L, "Mahshi",             60.0, "AVAILABLE");    //  5 — rest 2
        _seedDriver(2L, "Foul Sandwich",      30.0, "AVAILABLE");    //  6 — rest 2
        _seedDriver(3L, "Closed Item 1",      90.0, "UNAVAILABLE");  //  7 — rest 3
        _seedDriver(3L, "Closed Item 2",     100.0, "UNAVAILABLE");  //  8 — rest 3
        _seedDriver(3L, "Closed Item 3",     110.0, "UNAVAILABLE");  //  9 — rest 3
        _seedDriver(4L, "Falafel Sandwich",   25.0, "AVAILABLE");    // 10 — rest 4

        // ── Rides (8) ────────────────────────────────────────────
        // Distribution: user 1 (admin) = 1 ride; user 2 = 2; user 4 = 5
        // (so /preferences/dietary?minRides=5 matches user 4)
        _seedRide(1L, 1L, "PLACED",    100.00);  // ride 1 — admin
        _seedRide(2L, 1L, "DELIVERED",  50.00);  // ride 2 — user 2
        _seedRide(2L, 2L, "CONFIRMED",  75.00);  // ride 3 — user 2
        _seedRide(4L, 1L, "DELIVERED",  30.00);  // ride 4 — user 4
        _seedRide(4L, 2L, "DELIVERED",  40.00);  // ride 5 — user 4
        _seedRide(4L, 1L, "DELIVERED",  50.00);  // ride 6 — user 4
        _seedRide(4L, 2L, "DELIVERED",  60.00);  // ride 7 — user 4
        _seedRide(4L, 4L, "PLACED",     70.00);  // ride 8 — user 4
        // Optional ride_date (covers ?startDate=&endDate= filters)
        String[] _rideDateAliases = {"rideDate", "createdAt", "placedAt", "date"};
        _tryUpdateCol("Ride", _rideDateAliases, java.sql.Date.valueOf("2026-01-01"), 1L);
        _tryUpdateCol("Ride", _rideDateAliases, java.sql.Date.valueOf("2026-01-15"), 2L);
        _tryUpdateCol("Ride", _rideDateAliases, java.sql.Date.valueOf("2026-02-01"), 3L);
        _tryUpdateCol("Ride", _rideDateAliases, java.sql.Date.valueOf("2026-01-05"), 4L);
        _tryUpdateCol("Ride", _rideDateAliases, java.sql.Date.valueOf("2026-01-10"), 5L);
        _tryUpdateCol("Ride", _rideDateAliases, java.sql.Date.valueOf("2026-01-20"), 6L);
        _tryUpdateCol("Ride", _rideDateAliases, java.sql.Date.valueOf("2026-02-05"), 7L);
        _tryUpdateCol("Ride", _rideDateAliases, java.sql.Date.valueOf("2026-02-15"), 8L);
        // Optional metadata jsonb (covers /rides/metadata/search?key=note&value=true)
        String[] _rideMetaAliases = {"metadata", "details", "extra"};
        _tryUpdateJsonb("Ride", _rideMetaAliases,
            "{\"note\":\"true\",\"priority\":\"HIGH\",\"channel\":\"app\"}", 1L);
        _tryUpdateJsonb("Ride", _rideMetaAliases,
            "{\"note\":\"true\",\"channel\":\"web\"}", 2L);
        // Optional tracking_address column (text variant, if student stores as varchar)
        String[] _delAddrTextAliases = {"trackingAddress", "shippingAddress", "address"};
        _tryUpdateCol("Ride", _delAddrTextAliases, "1 Tahrir Sq, Cairo", 1L);
        _tryUpdateCol("Ride", _delAddrTextAliases, "5 Zamalek St, Cairo", 2L);
        // Optional tracking_address_id column (FK variant). Address ownership:
        // addresses 1-3 → user 1; address 4 → user 2; address 5 → user 4.
        String[] _delAddrFkAliases = {"trackingAddressId", "trackingAddress", "shippingAddressId", "addressId"};
        _tryUpdateCol("Ride", _delAddrFkAliases, 1L, 1L);  // ride 1 → addr 1 (user 1)
        _tryUpdateCol("Ride", _delAddrFkAliases, 4L, 2L);  // ride 2 → addr 4 (user 2)
        _tryUpdateCol("Ride", _delAddrFkAliases, 4L, 3L);  // ride 3 → addr 4 (user 2)
        _tryUpdateCol("Ride", _delAddrFkAliases, 5L, 4L);  // ride 4 → addr 5 (user 4)
        _tryUpdateCol("Ride", _delAddrFkAliases, 5L, 5L);  // ride 5 → addr 5 (user 4)
        _tryUpdateCol("Ride", _delAddrFkAliases, 5L, 6L);  // ride 6 → addr 5 (user 4)
        _tryUpdateCol("Ride", _delAddrFkAliases, 5L, 7L);  // ride 7 → addr 5 (user 4)
        _tryUpdateCol("Ride", _delAddrFkAliases, 5L, 8L);  // ride 8 → addr 5 (user 4)

        // ── Rides (8) ────────────────────────────────────────
        // Ride 1 has 4 items (2 PREPARED, 2 PENDING) — covers S3-F9
        // (ride details with item-status breakdown). Other rides get 1
        // item each so /rides/{id}/details and /rides/{id}/items return
        // non-empty for any positive id in 1..5.
        _seedRide(1L, 1L, 1,  80.0, "PREPARED");   // 1 — ride 1
        _seedRide(1L, 2L, 1, 100.0, "PREPARED");   // 2 — ride 1
        _seedRide(1L, 3L, 2,  60.0, "PENDING");    // 3 — ride 1
        _seedRide(1L, 4L, 1,  50.0, "PENDING");    // 4 — ride 1
        _seedRide(2L, 4L, 1,  50.0, "DELIVERED");  // 5 — ride 2
        _seedRide(3L, 5L, 1,  60.0, "DELIVERED");  // 6 — ride 3
        _seedRide(4L, 4L, 1,  50.0, "DELIVERED");  // 7 — ride 4
        _seedRide(5L, 5L, 1,  60.0, "DELIVERED");  // 8 — ride 5

        // ── Deliveries (8) ────────────────────────────────────────
        // Statuses spread; FK to rides 1..8.
        _seedTracking(1L, "PENDING");
        _seedTracking(2L, "DELIVERED");
        _seedTracking(3L, "IN_TRANSIT");
        _seedTracking(4L, "DELIVERED");
        _seedTracking(5L, "DELIVERED");
        _seedTracking(6L, "DELIVERED");
        _seedTracking(7L, "DELIVERED");
        _seedTracking(8L, "PENDING");
        ...
        _tryUpdateCol("Tracking", _driverAliases, "Ahmed",  1L);
        _tryUpdateCol("Tracking", _driverAliases, "Sara",   2L);
        _tryUpdateCol("Tracking", _driverAliases, "Omar",   3L);
        _tryUpdateCol("Tracking", _driverAliases, "Hassan", 4L);
        _tryUpdateCol("Tracking", _driverAliases, "Hassan", 5L);
        _tryUpdateCol("Tracking", _driverAliases, "Hassan", 6L);
        _tryUpdateCol("Tracking", _driverAliases, "Hassan", 7L);
        _tryUpdateCol("Tracking", _driverAliases, "Hassan", 8L);
        // Optional metadata jsonb — Hassan's deliveries with the spec's speeds
        String[] _delMetaAliases = {"metadata", "details", "extra"};
        _tryUpdateJsonb("Tracking", _delMetaAliases, "{\"driverName\":\"Hassan\",\"speed\":20,\"note\":\"true\"}", 4L);
        _tryUpdateJsonb("Tracking", _delMetaAliases, "{\"driverName\":\"Hassan\",\"speed\":30,\"note\":\"true\"}", 5L);
        _tryUpdateJsonb("Tracking", _delMetaAliases, "{\"driverName\":\"Hassan\",\"speed\":40,\"note\":\"true\"}", 6L);
        _tryUpdateJsonb("Tracking", _delMetaAliases, "{\"driverName\":\"Hassan\",\"speed\":50,\"note\":\"true\"}", 7L);
        _tryUpdateJsonb("Tracking", _delMetaAliases, "{\"driverName\":\"Hassan\",\"speed\":0,\"note\":\"true\"}",  8L);
        ...
        // ── Coupons (3) ────────────────────────────────────────────
        _seedCoupon("PRESEED10", "PERCENTAGE", 10.0, 100);
        _seedCoupon("PRESEED20", "FIXED",      20.0,  50);
        _seedCoupon("PRESEED5",  "PERCENTAGE",  5.0, 200);

        // ── payment_coupons junction ───────────────────────────────
        // Usage 4/2/1 across coupons 1/2/3 — covers /payments/coupons/top-used.
        _seedPaymentCoupon(1L, 1L);
        _seedPaymentCoupon(2L, 1L);
        _seedPaymentCoupon(3L, 1L);
        _seedPaymentCoupon(4L, 1L);
        _seedPaymentCoupon(5L, 2L);
        _seedPaymentCoupon(6L, 2L);
        _seedPaymentCoupon(7L, 3L);
    }
    // ─── end auto-seed hooks ───
    ...
    /** Canonical CRUD paths from seed_manifests/Uber_M2.json. */
     ...
     *
     * @param entitySingular manifest entity key, e.g. "Ride"
     * @return discovered base path, e.g. "/api/rides" or a student variant
     */
